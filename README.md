...
- `make up` – build and start all Docker services (API starts automatically).
- `make down` – stop and remove all Docker services.
- `make logs` – follow Docker logs.
- `make api` – show API access information (API runs automatically with `make up`).
- `make tests` – run the pytest test suite in Docker.
- `make lint` – run pylint in Docker on `app/`, `scripts/`, and `tests/`.
- `make etl-snap` – load the SNAP GitHub dataset into Neo4j (runs in Docker).
- `make etl-jobs` – load LinkedIn job listings into Neo4j (runs in Docker).
- `make etl-user-emb` – compute user embeddings based on features (runs in Docker).
